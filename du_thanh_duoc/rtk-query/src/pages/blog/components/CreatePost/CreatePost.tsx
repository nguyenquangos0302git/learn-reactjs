...
export default function CreatePost() {
  ...
  const { data, isFetching, refetch } = useGetPostQuery(postId, { skip: !postId, pollingInterval: 1000 })
  ...
  const errorForm: FormError = useMemo(() => {
    const errorResult = !postId ? addPostResult.error : updatePostResult.error
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!postId) {
        ...
        const result = await updatePost({ id: postId, body: formData as Post }).unwrap()
        // dispatch()
      }
  ...
  return (
    <form onSubmit={handleSubmit} onReset={() => {}}>
      ...
      <div className='mb-6'>
        {/* <label
          htmlFor='publishDate'
          className={`mb-2 block text-sm font-medium dark:text-gray-300 ${errorForm?.publishDate ? 'text-red-700' : 'text-gray-900'}`}
        > */}
        <label
          htmlFor='publishDate'
          className={classNames('mb-2 block text-sm font-medium dark:text-gray-300 text-gray-900', {
        ...
        <input
          type='datetime-local'
          id='publishDate'
          className={`block w-56 rounded-lg border p-2.5 text-sm focus:outline-none border-gray-300 bg-gray-50 text-gray-900 focus:border-blue-500 focus:ring-blue-500`}
          placeholder='Title'
          required
          value={formData.publishDate}
          onChange={(event) => setFormData((prev) => ({ ...prev, publishDate: event.target.value }))}
        ...
        {/* {errorForm?.publishDate && (
          <p className='mt-2 text-sm text-red-600'>
            <span className='form-medium'>{errorForm?.publishDate}</span>
          </p>
        )} */}
      </div>
